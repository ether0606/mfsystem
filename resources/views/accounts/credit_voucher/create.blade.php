@extends('layout.app')
@section('title','Credit Voucher Add')
@push('style')
<style>
select {
    padding: 3px 4px;
    ...
    @endif
        ...
        <div class="widget-box">
			<div class="widget-body">
				...
				<form action="{{route(currentUser().'.crvoucher.store')}}" method="post" class="form-search">
					@csrf
						<div class="row">
							...
							<div class="col-xs-12 col-sm-3">
								<div class="form-group @if($errors->has('particular')) has-error @endif">
									...
									<span class="block input-icon input-icon-right">
										<select class="width-100" name="credit">
										    @if($datas)
										        @foreach($datas as $d)
										            <option value="{{$d['table_name']}}~{{$d['table_id']}}~{{$d['value']}}">{{$d['value']}}</option>
										        @endforeach
										    @endif
										</select>
									</span>
...
<script>
    ...
    function get_head(code){
	    if($(code).val()!=""){
            $.getJSON( "{{route(currentUser().'.debit_get_head')}}",{'code':$(code).val()}, function(j){
	            if(j.length>0){
            		var data			= '';
            		var table_name 		= '';
            		var table_id 		= '';
