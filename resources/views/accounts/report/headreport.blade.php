...
<div class="row">
									...
									<select name="head_id" class="form-control">
									    @if($headlists)
									        @foreach($headlists as $h)
									            @if($h->masterhead_id)
									                <option value="{{$h->master_head->id}}-masterhead_id-{{$h->master_head->opening_balance}}">{{$h->master_head->head_name}}</option>
									            @elseif($h->subhead_id)
									                <option value="{{$h->sub_head->id}}-subhead_id-{{$h->sub_head->opening_balance}}">{{$h->sub_head->head_name}}</option>
									            @elseif($h->chieldheadone_id)
									                <option value="{{$h->chield_one->id}}-chieldheadone_id-{{$h->chield_one->opening_balance}}">{{$h->chield_one->head_name}}</option>
									            @elseif($h->chieldheadtwo_id)
									                <option value="{{$h->chield_two->id}}-chieldheadtwo_id-{{$h->chield_two->opening_balance}}">{{$h->chield_two->head_name}}</option>
									           @endif
									        @endforeach
									    @endif
								    </select>
    ...
    <div class="col-xs-12">
        ...
        <table class="table table-bordered table-hover" width="100%">
			...
			<tbody>
			    @php $balance=round($opening_bal,2);$deb=$cre=0; @endphp
			    @if($accData)
			        <tr>
						...
						<td>
						    ...
						    @elseif($accOldData->sum('dr')<$accOldData->sum('cr'))
						        ...
						        @php $balance+= ($accOldData->sum('cr') - $accOldData->sum('dr')); @endphp
						    @else
						            0
						    @endif
						</td>
